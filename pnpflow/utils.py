...
def postprocess(img, args):
    ...
    if args.model == "ot" or args.model == "gradient_step" or args.model == "diffusion":
        if args.dataset == "afhq_cat":
            ...
            normalization = getattr(args, 'normalization', 'minus1_plus1')
            if normalization == 'minus1_plus1':
                invTrans = v2.Normalize(mean=[-0.5 / 0.5], std=[1. / 0.5])
                img = invTrans(img)
            elif normalization == '0_1':
                pass
            elif normalization == '0_255':
                img = img / 255.0
        else:
