...
class PNP_FLOW(object):
    ...
    def grad_datafit(self, x, y, H, H_adj):
        ...
        #添加梯度步
            ...
            # This is the smooth term, which encourages neighboring pixels to have similar values.
            # alpha = 0.1
            # def regularization(x_old: torch.Tensor, alpha: float = 0.01) -> torch.Tensor:
            #     # 1. 4邻域均值x_bar（文档原始定义）
            #     x_old = x_old.to(torch.float32)
            #     kernel = torch.tensor([[[[0,1,0],[1,0,1],[0,1,0]]]], dtype=torch.float32, device=x_old.device)
            #     x_bar = F.conv2d(x_old, kernel, padding=1) / 4.0
            #     reg_gradient = alpha * (x_old - x_bar)
            #     return reg_gradient
            
            # gradient += regularization(x_clipped, alpha)
            return gradient

            # approximate method, using the tylor expansion of the gradient, which is more stable when x is close to 0
            # r = 1 - x
            # x_d = 0.0
            # n_terms = 10
            # for i in range(n_terms):
            #     x_d += r**i  # 累加(1-x)^0到(1-x)^7

            # gradient = self.args.sigma_noise * (x_d - y * (x_d ** 2))
            # # gradient = self.args.sigma_noise * (x_clipped - y) / torch.square(x_clipped)
            # alpha = 0.1
            # def regularization(x_old: torch.Tensor, alpha: float = 0.01) -> torch.Tensor:
            #     # 1. 4邻域均值x_bar（文档原始定义）
            #     x_old = x_old.to(torch.float32)
            ...
            # gradient += regularization(x, alpha)
        ...
        for batch in range(self.args.max_batch):
            ...
            with torch.no_grad():
                for count, iteration in enumerate(range(int(steps))):
                    ...
                    # This is a direct iterative method obtained based on Rafa's formula.
                    # def get_x_bar(x_old: torch.Tensor) -> torch.Tensor:
                    #     x_old = x_old.to(torch.float32)
                    #     kernel = torch.tensor([[[[0,1,0],[1,0,1],[0,1,0]]]], 
                    #                         dtype=torch.float32, device=x_old.device)
                    #     x_bar = F.conv2d(x_old, kernel, padding=1) / 4.0
                    #     return x_bar
                    # alpha = 1000
                    # eps = 1e-8
                    # denominator = self.args.sigma_noise + alpha * x ** 2 + eps
                    # term1 = self.args.sigma_noise / denominator * noisy_img
                    # term2 = (alpha * x ** 2) / denominator * get_x_bar(x)
                    # z = term1 + term2
                    # z = torch.clamp(z, 0, 1.0)
