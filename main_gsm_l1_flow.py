...
from pnpflow.methods.pnp_flow_gsm_l1 import PNP_FLOW
...
def parse_args() -> argparse.Namespace:
    ...
    gsm_defaults = {
        'use_gsm_prior': True,
        'gsm_prior_eps': 1e-3,
        'gsm_prior_log_domain': True,
        'gsm_prior_normalize': False,
        'gsm_prior_beta': 1e-3,
        'lr_gamma_pnp': 0.5,
        'gamma_clamp_after_denoise': True,
        'gamma_x_min': 1e-6,
    }
    for key, value in gsm_defaults.items():
        if not hasattr(cfg, key):
            cfg[key] = value
    for key in gsm_defaults.keys():
        cfg.dict_cfg_method[key] = cfg[key]
